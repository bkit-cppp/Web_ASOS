...
function MainNavbar() {
    ...
    const handleGenderClick = (item: GenderMenu) => {
        if (!isProductPage) {
            switchGender(item);
        }
    };
    ...
    const handleSubCategoryHover = (children: CategoryInGender[]) => {
        if (!isProductPage) {
            onShowSubCategory(children);
        }
    };
                            ...
                            {categories?.map((item, key) => {
                                return (
                                    <Button
                                        ...
                                        onMouseEnter={() => handleSubCategoryHover(item.children)}
                                        key={key}
